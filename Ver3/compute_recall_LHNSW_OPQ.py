...
OPQquery  = np.load("./input/HNSW_OPQ16/HNSW_OPQ16_Query.npy")
...
def get_cand( faiss_I, OPQquery, k):
    HNSW_weight_path = "./input/HNSW_OPQ16/HNSW_OPQ16_index/index_HNSW_{}.index"
    ...
    for i in faiss_I:
        ...
        HNSW_D, HNSW_I = HNSWindex.search(OPQquery,k)
        end_time = time.time()
...
if __name__ == '__main__':
    ...
    for i in range(query.shape[0]):
        # print(i)
        total_D, total_I, time_cost = get_cand(I[i], OPQquery[i], k)
    ...
    print(recall_score)
    np.savetxt("./output/HNSW_OPQ_recall.txt", recall_score)
    print(total_time)   
