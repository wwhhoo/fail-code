...
OPQquery  = np.load("./input/HNSWOPQ_with_OPQ/OPQ_Query.npy")
OPQquery  = np.float32(OPQquery)
HNSWquery = np.load("./input/HNSWOPQ_with_OPQ/HNSW_Query.npy")
...
def PQ_cand(PQ_data, PQ_ID, query, PQNN):
    ...
    PQ_weight_path = "./input/HNSWOPQ_with_OPQ/OPQ_index/index_OPQ.index"
    bits  = 8
    nlist = 2**bits
    PQ_data  = np.float32(PQ_data)
    # print(PQ_data.shape)
    # PQ_data  = np.reshape(PQ_data, PQ_data.shape[0])
...
def get_cand( faiss_I, query, OPQquery, PQNN, k):
    HNSW_weight_path = "./input/HNSWOPQ_with_OPQ/HNSW_index/index_HNSW_{}.index"
    HNSW_ID_path = "./input/HNSW_with_PQ/HNSW_data_ID/HNSW_data_ID_{}.npy"
    OPQ_data_path = "./input/HNSWOPQ_with_OPQ/OPQ_data/OPQ_data_{}.npy"
    ...
    for i in faiss_I:
        ...
        # if first:
        #     totalPQ_train_Data = np.concatenate((totalPQ_train_Data, PQ_data))
        #     totalPQ_train_ID = np.concatenate((totalPQ_train_ID, PQ_ID))
        # else:   
        #     totalPQ_train_Data = PQ_data
        #     totalPQ_train_ID = PQ_ID
        #     first = True
...
if __name__ == '__main__':
    ...
    for times in range(4):
        ...
        for i in range(query.shape[0]):
            ...
            total_D, total_I, HNSW_cost, PQ_cost, candidate = get_cand(I[i], HNSWquery[i], OPQquery[i], PQNN, k)
