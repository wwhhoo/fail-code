...
def OPQ_cand(faiss_I, query, PQNN, d):
    ...
    for i in faiss_I:
        ...
        index_ivfPQ.add_with_ids(OPQ_data,OPQ_ID)#(total_Data,total_ID)
        ...
        if  i == faiss_I[0]:
            ...
            total_Data = np.concatenate((total_Data,PQ_D))
            total_ID   = np.concatenate((total_ID,PQ_I))
        total_Data    = np.float32(total_Data)

    return total_Data, total_ID, time_cost, test_time
